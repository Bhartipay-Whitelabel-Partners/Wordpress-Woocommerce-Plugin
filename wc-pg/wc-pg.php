<?php
/*
 Plugin Name: Pg Gateway for WooCommerce
 Plugin URI: http://www.pg.com/
 Description: PG PG WooCommerce integration
 Version: 1.0
 Author: Pg <support@pg.com>
 Author URI: http://www.pg.com
 */

 add_action('plugins_loaded', 'woocommerce_pg_init', 0);

/**
 * [woocommerce_pg_init description]
 * @return [type] [description]
 */
function woocommerce_pg_init()
{
    if (! class_exists('WC_Payment_Gateway')) {
        return;
    }
    class WC_Gateway_PG extends WC_Payment_Gateway
    {
        public function __construct()
        {
            global $woocommerce;
            $this->id           = 'pg';
            $this->method_title = __('PG Payment Gateway', 'woocommerce');
            $this->icon         = apply_filters('woocommerce_pg_icon', plugins_url().'/wc-pg/images/logo.png');
            ...
            // $this->pg_request_url       = 'https://merchant.pg.com/crm/jsp/paymentrequest';
            $this->pg_request_url       = 'https://uat.pg.com/crm/jsp/paymentrequest';

            // Actions
            add_action('init', array( $this, 'check_pg_response' ));
            add_action('woocommerce_api_wc_gateway_pg', array( $this, 'check_pg_response' ));
            add_action('woocommerce_receipt_pg', array(&$this, 'receipt_page'));
        ...
        {
            echo '<h3>PG Payment Gateway</h3>';
            echo '<p>PG Payment Gateway is a payment product which makes it extremely safe and easy to pay online.</p>';
        ...
        public function process_payment($order_id)
        {
            ...
            $this->return_url = trailingslashit(home_url()).'?wc-api=WC_Gateway_PG&order_id='.$order->id;
            $this->init_settings();
        ...
        public function check_pg_response()
        {
        ...
        public function init_form_fields()
        {
            $this->form_fields = array(
                'enabled' => array(
                    ...
                    'type'    => 'checkbox', 
                    'label'   => __( 'Enable PG Gateway', 'woocommerce' ), 
                    'default' => 'yes'
                ), 
                'mode' => array(
                ...
                'title' => array(
                    ...
                    'default'     => __('PG Gateway', 'woocommerce'),
                    'desc_tip'    => true,
                ),
                'description' => array(
    ...
    function add_pg_gateway($methods)
    {
        $methods[] = 'WC_Gateway_PG';
        return $methods;
    }

    add_filter('woocommerce_payment_gateways', 'add_pg_gateway');
}
